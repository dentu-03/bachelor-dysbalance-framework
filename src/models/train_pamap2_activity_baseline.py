...
from src.project_paths import PROCESSED_DATA_DIR, REPORTS_DIR, MODELS_DIR
...
def load_subject_tensors(subject_ids: list[str]) -> tuple[np.ndarray, np.ndarray, pd.DataFrame]:
    """Load and concatenate PAMAP2 tensors for selected subjects."""
    ...
    for subject_id in subject_ids:
        x_path = base_dir / f"X_subject{subject_id}.npy"
        y_path = base_dir / f"y_subject{subject_id}.npy"
        meta_path = base_dir / f"metadata_subject{subject_id}.csv"
...
def make_label_names(labels: list[int]) -> list[str]:
    ...
    return [f"{label}\n{ACTIVITY_LABELS.get(int(label), 'unknown')}" for label in labels]
...
def main() -> None:
    ...
    print()
    print("Training MiniRocket baseline...")
    ...
    result_summary = {
        ...
        "task": "PAMAP2 activity classification",
        "split": "subject-wise",
    ...
    with open(report_dir / "pamap2_minirocket_baseline_summary.json", "w") as f:
        json.dump(result_summary, f, indent=2)

    with open(report_dir / "pamap2_minirocket_classification_report.json", "w") as f:
        json.dump(report_dict, f, indent=2)

    with open(report_dir / "pamap2_minirocket_classification_report.txt", "w") as f:
        f.write(report_text)

    cm_df = pd.DataFrame(cm, index=labels, columns=labels)
    cm_df.to_csv(report_dir / "pamap2_minirocket_confusion_matrix.csv")

    save_confusion_matrix_plot(
        cm=cm,
        labels=labels,
        out_path=report_dir / "pamap2_minirocket_confusion_matrix_raw.png",
        title=f"PAMAP2 MiniRocket Confusion Matrix\nAccuracy: {accuracy:.4f}",
        normalize=False,
    )
    ...
    save_confusion_matrix_plot(
        cm=cm,
        labels=labels,
        out_path=report_dir / "pamap2_minirocket_confusion_matrix_normalized.png",
        title=f"PAMAP2 MiniRocket Normalized Confusion Matrix\nAccuracy: {accuracy:.4f}",
        normalize=True,
    )

    joblib.dump(clf, model_dir / "pamap2_minirocket_subject_split.joblib")

    print()
    print(f"Saved reports to: {report_dir}")
    print(f"Saved raw confusion matrix PNG to: {report_dir / 'pamap2_minirocket_confusion_matrix_raw.png'}")
    print(f"Saved normalized confusion matrix PNG to: {report_dir / 'pamap2_minirocket_confusion_matrix_normalized.png'}")
    print(f"Saved model to: {model_dir / 'pamap2_minirocket_subject_split.joblib'}")
